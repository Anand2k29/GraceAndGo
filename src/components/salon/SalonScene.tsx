...
function Walls() {
  const wallMat = (
    <meshStandardMaterial color="#1a1715" roughness={0.85} metalness={0.05} />
  );
  return (
    <group>
      {/* back wall */}
    ...
    <group position={position}>
      ...
      {Array.from({ length: 8 }).map((_, i) => {
        ...
        return (
          <mesh
            key={i}
            position={[Math.cos(a) * 0.6, -0.85, Math.sin(a) * 0.6]}
          >
...
function CameraRig({ scroll }: { scroll: number }) {
  ...
  const waypoints = useMemo<{ p: THREE.Vector3; l: THREE.Vector3 }[]>(
    () => [
      ...
      {
        p: new THREE.Vector3(-4, 1.7, 2.5),
        l: new THREE.Vector3(-3.2, 1.4, -1.5),
      },
  ...
  useFrame((state) => {
    ...
    // Gentle camera sway (cinematic handheld/drone effect)
    const time = state.clock.getElapsedTime();
    const swayX = Math.sin(time * 0.5) * 0.12;
